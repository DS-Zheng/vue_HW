...
<template>
  <li class="flex items-center gap-2 text-2xl">
    <input type="checkbox" v-model="task.taskstatus">
    
    <router-link :to="{ name: 'detail', params: { auo: task.id } }">
      <span :class="{ 'taskstatus': task.taskstatus }">{{ task.title }}</span>
    </router-link>
    <button :data-taskid="task.id" @click="removeTask" class="btn btn-xs">
      <svg
